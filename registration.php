...
//AUTHENTICATION 
 if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    foreach ($error as $key => $value){
        // Registers the user using foreach loop
        if(empty($value)){
                ...
                <h1><?php echo _REGISTER; ?></h1>
                    <form role="form" action="registration.php" method="post" id="login-form" autocomplete="off">
                        ...
                        <div class="form-group">
                            ...
                            value = "<?php echo isset($username) ? $username: '' //username displays on the textfield when clicking the submit button ?>" 
                            >
                            <p><?php echo isset($error['username']) ? $error['username']: '' //Displays the error?></p>
                        </div>
                         <div class="form-group">
                            <label for="email" class="sr-only">Email</label>
                            <input type="email" name="email" id="email" class="form-control" placeholder="<?php echo _EMAIL; ?>" autocomplete="on" 
                            value = "<?php echo isset($email) ? $email: '' //username displays on the textfield when clicking the submit button ?>" >
                            <p><?php echo isset($error['email']) ? $error['email']: '' //Displays the error ?></p>
                         ...
                         <div class="form-group">
                            ...
                            <p><?php echo isset($error['password']) ? $error['password']: '' //Displays the error ?></p>
